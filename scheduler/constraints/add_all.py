...
def add_constraints(config: ScheduleConfig, s_model: ScheduleModel):
    ...
    SaturdayConstraint(config, s_model).add()
    FridaySaturdayConstraint(config, s_model).add()
    NoSoloConstraint(config, s_model).add()
    NotAllowConstraint(config, s_model).add()
